...
public class DebugDrawEditor : EditorPlugin
{
    ...
    SubViewportContainer spatial_viewport = null;

    public override void _EnterTree()
    {
        CreateAutoFind();

        if (!IsConnected(SignalName.SceneChanged, Callable.From<Node>(OnSceneChanged)))
    ...
    public override void _ExitTree()
    {
        ...
        if (IsConnected("scene_changed", Callable.From<Node>(OnSceneChanged)))
    ...
    void FindViewportControl()
    {
        ...
        if (spatial_editor.GetClass() == "SpatialEditor")
        {
            ...
            spatial_viewport = get(spatial_editor, 0)?.GetChild<SubViewportContainer>(0);
        }
